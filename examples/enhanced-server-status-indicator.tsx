 ...
 * 
 * An advanced status indicator that shows the health of backend services
...
export interface EnhancedServerStatusIndicatorProps {
  position?: 'top-right' | 'top-left' | 'bottom-right' | 'bottom-left';
  showLabel?: boolean;
}

export const EnhancedServerStatusIndicator = ({ 
  position = 'bottom-right',
  showLabel = true 
}: EnhancedServerStatusIndicatorProps) => {
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      try {
        ...
        if (isMounted) {
          // Check if launcher is available when main API is down
          checkLauncherStatus();
              ...
              {Object.entries(status.components).map(([name, info]) => (
                <div key={name} className="flex justify-between">
                  ...
                  <span className={info.status === 'available' || info.status === 'connected' ? 'text-green-500' : 'text-gray-500'}>
                    ...
                    {name === 'indicators' && ` (${info.count} loaded)`}
                  </span>
                </div>
              ))}
...
export default EnhancedServerStatusIndicator;
